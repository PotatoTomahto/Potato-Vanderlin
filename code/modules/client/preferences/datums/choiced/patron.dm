...
/datum/preference/choiced/patron/handle_link(datum/preferences/prefs, mob/user)
	...
	to_chat(user, span_purple("Patron: [pref_patron]"))
	to_chat(user, span_purple("Domain: [pref_patron.domain]"))
	to_chat(user, span_purple("Background: [pref_patron.desc]"))
	to_chat(user, span_purple("Flawed aspects: [pref_patron.flaws]"))
	to_chat(user, span_purple("Likely Worshippers: [pref_patron.worshippers]"))
	to_chat(user, span_red("Considers these to be Sins: [pref_patron.sins]"))
	to_chat(user, span_green("Blessed with boon(s): [pref_patron.boons]"))
	if(pref_patron.drawbacks)
		to_chat(user, span_red("Cursed with drawback(s): [pref_patron.drawbacks]"))
